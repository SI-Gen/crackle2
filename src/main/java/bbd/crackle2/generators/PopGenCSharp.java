...
public class PopGenCSharp extends Generator {
	...
	public static void generateStructs(Module module, String output, PrintWriter outLog) {
		try {
			...
			try {
				...
				outData.println("// Generated by PopGenCSharp.java - generateStructs.");
				outData.println();
	...
	public static void generateInterfaces(Module module, String output, PrintWriter outLog) {
		...
		try {
			String outFilePath = output + "I" + module.name + ".cs";
			outLog.println("Generating File: " + outFilePath);
			OutputStream outFile = new FileOutputStream(outFilePath);
			PrintWriter outData = new PrintWriter(outFile);
			try {
				outData.println("// This code was generated, do not modify it, modify it at source and regenerate it.");
				outData.println("// Generated by PopGenCSharp.java - generateInterfaces.");
				outData.println();
				outData.println("using System;");
				outData.println("using Bbd.Idl2;");
				//generateUsings(module, outData);
	...
	public static void generateClasses(Module module, String output, PrintWriter outLog) {
		try {
			...
			try {
				outData.println("// This code was generated, do not modify it, modify it at source and regenerate it.");
				outData.println("// Generated by PopGenCSharp.java - generateClasses.");
				outData.println();
				outData.println("using System;");
				...
				outData.println(
						"    public " + module.name + "(Header header, string host, string service, int timeOut)");
	...
	public static void generateProxies(Module module, String output, PrintWriter outLog) {
		try {
			String outFilePath = output + module.name + "Proxy.cs";
			outLog.println("Generating File: " + outFilePath);
			OutputStream outFile = new FileOutputStream(outFilePath);
			PrintWriter outData = new PrintWriter(outFile);
			try {
				outData.println("// This code was generated, do not modify it, modify it at source and regenerate it.");
				outData.println("// Generated by PopGenCSharp.java - generateProxies.");
				outData.println();
				outData.println("using System;");
				outData.println("using Bbd.Idl2.Rpc;");
				generateUsings(module, outData);
				...
				outData.println(
						"    public " + module.name + "Proxy(Header header, string host, string service, int timeOut)");
				outData.println("    {");
		...
		for (int i = 0; i < module.structures.size(); i++) {
			...
			for (int j = 0; j < structure.fields.size(); j++) {
				...
				if (n > 0) {
					...
					outData.println("    [Field(Size=" + n + ")] byte[] _fill" + fillerNo + " = new byte[" + n + "];");
					fillerNo++;
					relativeOffset += n;
				}
				outData.println("    " + field.type.csAttribute(field.isStruct(module)) + "public "
	...
	public static boolean generateCommon(Module module, Prototype prototype, int no, PrintWriter outData,
			String extra1) {
		Parameter pd = null;
	...
	public static void generateCall(Module module, Prototype prototype, int no, PrintWriter outData) {
		...
		if (hasReturn) {
			outData.println(
					"        " + prototype.type.netName() + " result = new " + prototype.type.netName() + "();");
		...
		for (int i = 0; i < prototype.outputs.size(); i++) {
			...
			if (pd.isOutput == true && pd.isInput == false) {
				if (pd.isArray) {
					outData.println(
							"        " + pd.name + " = new " + pd.type + "[" + lowerFirst(pd.outputSizeValue) + "];");
			...
			if (pd.type.compareTo("string") != 0)
				outData.println("        " + pd.name + " = (" + pdType + ") Get(" + pd.name + ", " + align8 + ");");
			else
				outData.println("        " + pd.name + "= GetString(" + pd.outputSizeValue + ", " + align8 + ");");
		}
		if (hasReturn)
	...
	public static void generateIDL2PythonCall(Module module, PrintWriter outData) {
		...
		outData.println("  def __init__(self, name, type, isInput=False, isOutput=False):");
		outData.println("    self.name = name");
		outData.println("    self.type = type");
		...
		outData.println("  __slots__ = ['name', 'struct', 'table', 'type', 'extended', 'input', 'output']");
		outData.println("  def __init__(self, name, struct, table, type, extended, input, output):");
		outData.println("    self.name = name");
	...
	public static void generateStructure(Structure structure, PrintWriter outData) {
		outData.println();
		outData.print("structures.append(Structure('" + structure.name + "', " + structType(structure.codeType) + ",");
		if (structure.fields.size() == 0) {
			outData.println(" [] ) )");
			return;
		}
		...
		for (int i = 0; i < structure.fields.size(); i++) {
			...
			outData.println("  " + square + " Field('" + field.name + "', Type('" + field.type.name + "', "
	...
	public static void generatePrototype(Prototype prototype, PrintWriter outData) {
		...
		outData.print(" Type('" + prototype.type.name + "', " + fieldType(prototype.type.typeof) + ", "
